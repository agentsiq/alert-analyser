...
class AgentRunner:
    ...
    The Anthropic API key is resolved at call time: if the caller passes
    X-Anthropic-Key header or a DB-stored key, that takes priority; otherwise
    ...
    async def run(
        ...
        if not resolved_key:
            raise RuntimeError(
                ...
                "Set ANTHROPIC_API_KEY env var, POST /settings with api_key, "
                "or pass X-Anthropic-Key header."
            )
